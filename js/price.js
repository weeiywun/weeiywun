...
function initPriceSearch() {
  ...
  document.getElementById('price-results').innerHTML = '<div class="text-txt-3 text-[13px] py-4">輸入商品名稱開始查詢，例如：北耆王、天麻、枸杞、特炊芎…</div>';
}
...
function renderPriceSearch() {
  ...
  if (!q) { document.getElementById('price-results').innerHTML = '<div class="text-txt-3 text-[13px] py-4">輸入商品名稱開始查詢…</div>'; return; }
  ...
  document.getElementById('price-results').innerHTML = `
    <div class="grid grid-cols-3 gap-2.5 mb-4">
      ...
      <div class="metric"><div class="metric-label">價格範圍</div><div class="metric-value text-base">$${minP.toLocaleString()} ~ $${maxP.toLocaleString()}</div></div>
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          ${hits.map(h=>`<tr>
            <td>${h.date}</td>
            <td class="font-medium">${h.vendor}</td>
            <td>${h.name}</td>
            <td class="text-txt-2">${h.qty}</td>
            <td class="font-semibold text-info">$${h.price.toLocaleString()}</td>
            <td class="text-txt-2">$${Math.round(h.qty*h.price).toLocaleString()}</td>
          </tr>`).join('')}
        </tbody>
      </table>
    </div>`;
